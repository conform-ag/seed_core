...
frappe.ui.form.on('Seed Core Settings', {
    refresh: function (frm) {
        ...
        if (frm.doc.site_type === 'Subsidiary' && frm.doc.parent_site_url) {
            ...
            frm.add_custom_button(__('Sync Varieties'), function () {
                frappe.call({
                    method: 'seed_core.api.sync.pull_varieties_from_parent',
                    freeze: true,
                    freeze_message: __('Syncing seed varieties from parent...'),
                    callback: function (r) {
                        if (r.message) {
            ...
            frm.add_custom_button(__('Sync Targets'), function () {
                frappe.call({
